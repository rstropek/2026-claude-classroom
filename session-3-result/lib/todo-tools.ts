...
import { todoSchema } from "ai-tutor-contract";
 ...
 * /api/todos so all show the same thing. `text` narrows it to titles containing
 * that text, case-insensitively (SQLite's `lower` folds ASCII only).
 * created_at has millisecond precision, so the list comes back in insertion
 * order; id breaks the rare tie so the order is stable across queries.
 */
export function listTodosFor(db: TodoDb, userId: string, text?: string) {
  const conditions: SQL[] = [eq(todos.userId, userId)];
  if (text) {
    // instr rather than LIKE, so `%` and `_` in the filter match literally.
    conditions.push(sql`instr(lower(${todos.title}), lower(${text})) > 0`);
  }
...
/** Inserts one item owned by `userId`; the title is stored trimmed. */
...
/**
 * Sets `done` on one item, filtered by owner as well as id, so another user's
 * id reads as no such item: the result is null rather than an error.
 */
...
/**
 * The tutor's tools over the queries above. Every statement is filtered by the
 * context's `userId`, so a row belonging to another student is
